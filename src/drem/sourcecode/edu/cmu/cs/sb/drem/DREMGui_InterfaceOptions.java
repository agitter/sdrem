 ...
 */
public class DREMGui_InterfaceOptions extends JPanel implements ChangeListener, ItemListener, ActionListener
{
	...
	public DREMGui_InterfaceOptions(JFrame theFrame, DREMGui theDREMGui)
	{
		...
		colorPanel.setBackground(new Color((float) 0.0,(float) 1.0,(float) 0.0,(float) 0.4));
		...
		JLabel theTopLabel = new JLabel("             Scale Y-axis by the factor:              ");
		...
		pvalLabel = new JLabel("Y-axis scale factor is "+doubleToSz(theDREMGui.dscaley));
		labelPanel.setAlignmentX(Component.LEFT_ALIGNMENT);
		labelPanel.add(pvalLabel);
		labelPanel.setBackground(Color.white);
		add(labelPanel);

		JLabel theTopLabelX = new JLabel("             Scale X-axis by the factor:              ");
		JPanel topPanelX = new JPanel();
		topPanelX.setAlignmentX(Component.LEFT_ALIGNMENT);
		topPanelX.add(theTopLabelX);
		topPanelX.setBackground(new Color((float) 0.0,(float) 1.0,(float) 0.0,(float) 0.4));
		add(topPanelX);
		...
		pvalLabelX = new JLabel("X-axis scale factor is "+doubleToSz(theDREMGui.dscalex));
		...
		JLabel theXLabel = new JLabel("           X-axis scale should be:            ");
		add(theXLabel);
		...
		JLabel theNodeLabel = new JLabel("             Scale node areas by the factor:              ");
		JPanel nodeLabelPanel = new JPanel();
		nodeLabelPanel.setAlignmentX(Component.LEFT_ALIGNMENT);
		nodeLabelPanel.add(theNodeLabel);
		nodeLabelPanel.setBackground(new Color((float) 0.0,(float) 1.0,(float) 0.0,(float) 0.4));
		add(nodeLabelPanel);
		...
		JPanel nodelabelPanel = new JPanel();
		nodeLabel = new JLabel("Node area scale factor is "+doubleToSz(theDREMGui.dnodek));
		nodelabelPanel.setAlignmentX(Component.LEFT_ALIGNMENT);
		...
		breakPanel.setBackground(new Color((float) 0.0,(float) 1.0,(float) 0.0,(float) 0.4));
		add(breakPanel);

		attachBox = 
			new JCheckBox("Hide All Labels When Hiding Nodes");
	...
	public void actionPerformed(ActionEvent e) 
	{
		...
		{
			String szMessage = 
				"The first option is 'Gene colors should be based on edge' determines the color of "+
	...
	public void stateChanged(ChangeEvent e) 
	{
		...
		if (!source.getValueIsAdjusting()) 
		{
			if (source == theSlider)
			{
				...
				pvalLabel.setText("Y-axis scale factor is "+doubleToSz(theDREMGui.dscaley));
				theDREMGui.drawmain();
			}
			else if (source == theSliderX)
			{
				theDREMGui.dscalex = source.getValue()/10.0;
				pvalLabelX.setText("X-axis scale factor is "+doubleToSz(theDREMGui.dscalex));
				theDREMGui.drawmain();
			}
			else if (source == theNodeSlider)
			{
				...
				nodeLabel.setText("Node area scale factor "+doubleToSz(theDREMGui.dnodek));
				theDREMGui.drawmain();
			}
