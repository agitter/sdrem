 ...
 * The L1 penalty promotes sparsity in the classifier
 * Implementation based on the following paper 
 * Krishnapuram, B., Figueiredo, M., Carin, L., and Hartemink, A. 
 * Sparse Multinomial Logistic Regression: Fast Algorithms and Generalization Bounds.
 * IEEE Transactions on Pattern Analysis and Machine Intelligence (PAMI), 27, June 2005. pp. 957-968.
 */
public final class DREM_FastLogistic2
{
	static double EPSILON1 =0.01;
	static int MAXITERATIONS = 1000;
	static double LAMBDA = 1;

	/** Each of the n rows is a gene and gives the interaction value for TFs regulating the gene
	 * Each gene is duplicated numclasses number of times. */
	double[][] traindata;
	/** Each of the n rows is a gene and gives the index of TFs regulating the gene
	 * Each gene is duplicated numclasses number of times. */
	int[][] traindataindex;
	/** Each row is a TF and gives the index of genes the TF regulates.
	 * Each row beyond the true number of TFs represents a duplicate copy
	 * of the original TF */
	...
	double[] dyerror;
	/**
	 * In the binary case, caches the value
	 * exp(w * x) for one class and each gene.
	 * Updated as the weights are iteratively improved.
	 * Dimension:<br>
	 * [num training data points]*/
	double[] dipcache;
	/** In the multinomial case, caches the value
	 * exp(w * x) for each class and gene.
	 * Updated as the weights are iteratively improved. 
	 * Dimensions:<br>
	 * [num classes-1][num training data points]*/
	double[][] dipcachemulti;
	/** Gives the likelihood of a gene going through a state, which is
	 * equivalent to the likelihood of a gene belonging to a particular class. 
	 * These weights are not related to the weight vector w in the paper.  */
	double[] dtrainweight; 
	/** Not used  */
	double[] dbeta;
	/**Lambda*/
	double dridge = LAMBDA;
	/**The weights (w in the paper).
	 * dcoeff[0] is the intercept
	 * Dimension:<br>
	 * [(num TFs + 1)*(num classes - 1)], which is d*(m-1) */
	double[] dcoeff;
	/**The number of classes (m in the paper)*/
	int numclasses;
	/**Size of the weight vector (w) to be learned:  d*(m-1) */
	int numels;
	/**The weight of the more recently updated coefficient in the previous iteration*/
	double doldcoeff;
	/**Number of data points in the training data*/
	int numrows;
	/**Negative definite matrix that lower bounds the Hessian
	 * of the weights  Only the diagonal is stored. 
	 * Dimension:<br>
	 ...
	 * [(num TFs + 1)*(num classes - 1)], which is d*(m-1) */
	double[] g;
	/**num features (d in the paper).
	 * Dimension:<br> [num TFs + 1], the +1 is for the intercept*/
	int numcols;
	 ...
	 */
	 ...
	 public DREM_FastLogistic2(int[][] traindataindex, double[][] traindata, 
			 int[][] traindataTFindex, double[][] traindataTF,
			 int[] y,  int numclasses, double[] dcoeff, int numbits)
	 {
	 ...
	 public DREM_FastLogistic2(int[][] traindataindex, double[][] traindata, 
			 int[][] traindataTFindex, double[][] traindataTF,
			 int[] y, double[] dtrainweight, int numclasses, int numbits)
	 {
		 this.traindataindex = traindataindex;
		 this.traindata = traindata;
		 this.traindataTFindex = traindataTFindex;
	 ...
	 public String saveClassifier(String[] tfNames)
	 {
		 ...
		 for (int nrow = 0; nrow < dcoeff.length; nrow++)
		 {
			 ...
			 {
				 szbuf.append(tfNames[nrow%(tfNames.length+1)-1]+"\t"+dcoeff[nrow]);	     
			 } 
			 szbuf.append("\n");
		 }
	 ...
	 public void reinit(int[][] traindataindex, double[][] traindata, 
			 int[][] traindataTFindex, double[][] traindataTF,
			 int[] y, double[] dtrainweight, int numclasses)
	 {
		 this.traindataindex = traindataindex;
		 this.traindata = traindata;
		 this.traindataTFindex = traindataTFindex;
		 ...
		 if ((this.dtrainweight == null)||(this.dtrainweight.length != dtrainweight.length))
		 {
			 dyerror = new double[dtrainweight.length];
	 ...
	 /**
	  * If abs(da)-ddelta less than or equal to 0 returns 0
	  * otherwise if da is non-negative returns abs(da)-ddelta
	  * and otherwise returns -(abs(da)-ddelta)
	  */
	 ...
	 public void computeG(int nel, boolean bnew, boolean bfirsttime)
	 {
		 ...
		 if ((bnew)||(nelmod==0))
		 {
			 ...
			 else
			 {
				 ...
				 if (nelmod == 1)
				 {
					 ...
					 {
						 g[nel] += dyerror[traindataTFindex[0][ntfindex]]*traindataTF[0][ntfindex];
					 }
				 ...
				 else
				 {
					 ...
					 for (int ntfindex = 0; ntfindex < traindataTFindex[nelmod2].length; ntfindex++)
					 {
						 ...
						 {
							 double dprod = dcoeffdiff * traindataTF[nelmod2][ntfindex];
		 ...
		 if (Double.isNaN(g[nel]))
		 {
			 System.out.println("computeG\t"+g[nel]+"\t"+dcoeff[nel]+"\t"
					 +dridge+"\t"+nel+"\t"+bnew+"\t"+bfirsttime);
		 }
	 }

	 /**
	  * Updates the value of the G matrix for the binary case.  Based on Equation 9
	  * in Krishnapuram et al.
	  * @param bnew means the coefficient changed on the previous coordinate or first time.
	  * If didn't change then can keep previous errors just update current coefficient
	  * @param bfirsttime means it is the first pass through the coefficients
	  * @param nel The index of G to be updated.  Ranges from 0 to numcols-1
	  */
	 public void computeGbin(int nel, boolean bnew, boolean bfirsttime)
	 {
		 ...
		 { 
			 ...
			 if (nel == 0)
			 {
				 ...
				 {
					 //already have been around at least once, updating change from last coordinate
					 // Need to determine what was changed when the last element of the w vector
					 // was updated and update the 0th element of g accordingly 
					 double dcoeffdiff = dcoeff[dcoeff.length-1]-doldcoeff;
					 double dcoeffdiffexp = Math.exp(dcoeffdiff);
					 int nlastrow = 0;
					 for (int ntfindex = 0; 
					 ntfindex < traindataTFindex[nlastcolindex].length; ntfindex++)
					 {
						 ...
						 {
							 // If the interaction value isn't 1 or -1 we can't reuse
							 // Math.exp(dcoeffdiff) and have to recompute the
							 // exponent
							 double dprod = dcoeffdiff * traindataTF[nlastcolindex][ntfindex];
			 ...
			 else
			 {
				 ...
				 {
					 //constant always before this
					 // i.e. the last coefficient to be updated was that for the intercept
					 double dcoeffdiff =  dcoeff[0]-doldcoeff;
					 double dcoeffdiffexp = Math.exp(dcoeffdiff);
					 for (int nrow = 0; nrow < traindata.length; nrow++)
					 {
						 // The interaction value (training data value) is implicitly 1 
						 dipcache[nrow] *= dcoeffdiffexp;
				 ...
				 else
				 {
					 ...
					 { 
						 ...
						 {
							 double dprod = dcoeffdiff * traindataTF[nel2][ntfindex];
							 dipcache[nrow] *= Math.exp(dprod);
						 }
					 ...
					 {
						 g[nel] += dyerror[traindataTFindex[nel1][ntfindex]]*traindataTF[nel1][ntfindex];
					 }
				 }
			 }
		 }
	 ...
	 public boolean nextw(int nel,boolean bnew, boolean bfirst)
	 {
		 ...
		 else
		 {
			 ...
			 {
				 dcoeff[nel] = soft(dcoeff[nel] - g[nel]/B[nbindex], -dridge/B[nbindex]); 
			 }
		 }

		 if (Double.isNaN(dcoeff[nel]))
		 {
			 System.out.println("nextw\t"+dcoeff[nel]+"\t"+g[nel]+"\t"+B[nbindex]+"\t"+dridge);
		 }
		 return (doldcoeff != dcoeff[nel]);
	 }

	 /**
	  * For the binary case updates the value of dcoeff[nel].  Based on Equation 15
	  * in Krishnapuram et al.
	  */
	 ...
	 { 
		 ...
		 {	    
			 ...
			 {
				 dcoeff[nel] = soft(dcoeff[nel] - g[nel]/B[nel], -dridge/B[nel]); 
			 }
		 }
	 ...
	 private void computeB()
	 {
		 ...
		 for (int nrow = 0; nrow < traindataindex.length; nrow++)
		 {
			 ...
			 {
				 B[traindataindex[nrow][nrowindex]+1] 
				   += traindata[nrow][nrowindex] * traindata[nrow][nrowindex]*dtrainweight[nrow]; 
			 }
		 }

		 // Every element on the diagonal of the matrix formed by:
		 // -1/2 * [I - 1*(1)^T / m]
		 // has value dmI.  Based on the first part of Equation 8 in Krishnapuram et al.
	  ...
	  * Returns a probability distribution the classifier gives to each class for the provided instance
	  */
	 public double[] distributionForInstance(int[] theInstanceIndex, double[] theInstance, int nrow)
	 {
		 double[] dist = new double[numclasses];
		 if (numclasses == 2)
		 {
	 ...
	 /**
	  * Calls either trainbin or trainmulti to train the parameters of the classifier
	  */
	 public void train()
	 {
		 if (numclasses == 2)
	 ...
	 {
		 // The B matrix only needs to be calculated once since it depends only on
		 // m and x
		 computeB();
		 double dolddev =0;
