...
public class DREMGui_EdgeTable extends JPanel implements ActionListener
{
	 ...
	 */
	public DREMGui_EdgeTable(DREMGui theDREMGui,JFrame theFrame, DREM_Timeiohmm theTimeiohmm,
			DREM_Timeiohmm.Treenode ptr, int npathID,int ndepth, int nchild, int ntype,boolean broot)
	{
		//assuming that if called with root node then only one child so stats for root will be same as first child
		...
		int numtf =  theTimeiohmm.tfNames.length;
		int nsize = theTimeiohmm.hsUniqueInput.size();
		if (theTimeiohmm.hsUniqueInput.contains(new Integer(0)))
		{
			nsize--;
		}
		numrows = numtf * nsize;
		...
		{
			for (int nel = 0; nel < theTimeiohmm.dBindingSigns.length; nel++)
			{
				if (theTimeiohmm.dBindingSigns[nel] != 0)
				{
					tabledata[nrowindex][0] = theTimeiohmm.tfNames[nrow]+" "+ theTimeiohmm.dBindingSigns[nel];
					tabledata[nrowindex][1] = ""+
					theTimeiohmm.filteredClassifier.nBaseCount[nrow][theTimeiohmm.filteredClassifier.getFeatureIndex(theTimeiohmm.dBindingSigns[nel])];

					tabledata[nrowindex][2+nsplitoffset] =  ""+ ptr.ncountvals[nrow][nchild][nel];
					tabledata[nrowindex][3+nsplitoffset] = nf2.format(ptr.dexpectEdgeFull[nchild][nrowindex]);
					tabledata[nrowindex][4+nsplitoffset] = nf2.format(ptr.ddiffEdgeFull[nchild][nrowindex]);
					tabledata[nrowindex][5+nsplitoffset] = 
						DREMGui_EdgeTable.doubleToSz(ptr.dpvalEdgeFull[nchild][nrowindex]);


					if (bsplit)
					{
						tabledata[nrowindex][2] = ""+ptr.ncountTotals[nrow][nel];
						tabledata[nrowindex][7] = nf2.format(ptr.dexpectEdgeSplit[nchild][nrowindex]);
						tabledata[nrowindex][8] = nf2.format(ptr.ddiffEdgeSplit[nchild][nrowindex]);
						tabledata[nrowindex][9] = DREMGui_EdgeTable.doubleToSz(ptr.dpvalEdgeSplit[nchild][nrowindex]);
						...
						{
							tabledata[nrowindex][10] = ""+nf2.format(100*(double) ptr.ncountvals[nrow][nchild][nel]/
	...
	/**
	 * Helper function that adds information displayed at the bottom of the table information window
	 */
	private void addBottom()
	{
		...
		{
			szcountLabel += " ("+
			nf2.format(100*(double) ptr.nextptr[nchild].numPath/ptr.numPath)+"% of split genes)";
		}
		...
		if (broot)
		{
			szInfo = "Path output distribution at "
				+theTimeiohmm.theDataSet.dsamplemins[0]+" is Normal(mu ="+
				nf.format(ptr.dmean)+",sigma = " +nf.format(ptr.dsigma)+")";
		}
		else
		{
			szInfo = "Path output distribution at "
				+theTimeiohmm.theDataSet.dsamplemins[ptr.ndepth+1]+" is Normal(mu ="+
				nf.format(ptr.nextptr[nchild].dmean)+",sigma = " +nf.format(ptr.nextptr[nchild].dsigma)+")";
		}
		...
		Integer colorID = (Integer) theDREMGui.htLineIDtoColorID.get(ndepth+";"+npathID);
	...
	public void actionPerformed(ActionEvent e) 
	{
		...
		else if (szCommand.equals("color"))
		{
			...
			if (newColor != null)
			{
				...
				ArrayList linesToUpdate = (ArrayList) theDREMGui.htColorIDtoLinesList.get(colorID);
				...
				for (int nindex = 0; nindex < nsize; nindex++)
				{
					...
					{
						theDREMGui.htColors.put(theDREMGui.theSelectedRec.selectedNode,newColor);
					}
				}

				ArrayList circlesToUpdate = (ArrayList) theDREMGui.htColorIDtoCircleList.get(colorID);
