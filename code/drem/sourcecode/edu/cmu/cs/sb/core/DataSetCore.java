...
public class DataSetCore
{
	...
	/**
	 *Contains genes filtered (i.e. removed from the data set).  Maps these gene names to the list of probe IDs associated with it
	 */
	public HashMap htFiltered;
	...
	/**
	 *Present/missing data for one data set
	 *First dimension is gene
	 *Second dimension is spot
	 *Third is present/missing value
	 */
	 ...
	 *First dimension is gene
	 *Second dimension is spot
	 *Third is expression value (in log ratio form against time zero)
	 */
	 ...
	 */
	private DataSetCore(String szInputFile, double[][][] genespottimedata,double[][][][] generepeatspottimedata,
			int[][][] genespottimepma,int[][][][] generepeatspottimepma, double[][] data,int[][] pmavalues, 
			String[] probenames, String[] genenames, double dthresholdvalue, boolean btakelog,
			boolean bspotincluded,boolean badd0, String[] dsamplemins, double[] sortedcorrvals,
			int nmaxmissing, double dmincorrelation, HashMap htFiltered, String szProbeHeader, String szGeneHeader,
			boolean bmaxminval, String[] otherInputFiles,boolean bfullrepeat)
	{
	...
	{  
		...
		HashSet htNotFiltered = new HashSet();
		if (genenames != null)
		{
		...
		for (int nrow = 0; nrow < nextrasize; nrow++)
		{
			...
			if (!htNotFiltered.contains(szextragene))
			{
				...
				{
					//the pre-filtered file may contain the same spot ID as a gene that was filtered
					...
					if (!bfound)
					{
						//probe is new adding it to the list of probes for the gene
						htFiltered.put(szextragene, szProbe+";"+szExtraProbe);
					}
				}
			}
		}
	...
	throws IOException, FileNotFoundException, IllegalArgumentException
	{
		...
		//checks if every col ends with a tab then we assume we have one less column
		boolean balltabend = true;

		//stores the expression data into alInputFile
		ArrayList alInputFile = new ArrayList();
		...
		for (int nrow = 0; nrow < numrows; nrow++)
		{
			...
			if (bspotincluded)
			{
				...
				{       
					...
					{ 
						...
						if (brepeatset)
						{
							szmsg = "Spot name "+sztoken+" in repeat/comparison is not unique";
						}
						else
						{
							szmsg = "Spot name "+sztoken+" is not unique"+
							"\nConsider unchecking 'Spot IDs included in the data file'";
						}
					...
					else
					{
						...
						probenames[nrow] = sztoken.trim().toUpperCase(Locale.ENGLISH); 
					}
			...
			else
			{
				...
				{
					if ((sztoken.charAt(0) == '\"') && (sztoken.charAt(sztoken.length()-1)=='\"'))
					{ 
						//strings quotes
				...
				for ( ; ncol < numcols; ncol++)
				{
					...
					{  
						...
						catch (NumberFormatException pe)
						{
							...
							else if (((ncol==0)||((ncol==1)&&(badd0))) &&(!bspotincluded))
							{
						...
						{
							//negative values are counting as missing when not already in log-space
							pmavalues[nrow][ncol] = 0;
						}
						else
						{
	...
	/////////////////////////////////////////////////////////////////
	/**
	 *Removes those rows which are the duplicate of another row. Stores in htgenenames for each gene
	 *name the index of all rows associated with it
	 */
	public DataSetCore filterDuplicates()
	{
		...
		Hashtable htgenenames = new Hashtable();
		//built hashtable of name to index
		for (int nrow = 0; nrow < numrows; nrow++)
		{
			ArrayList indicies = (ArrayList) htgenenames.get(genenames[nrow]);  
			if (indicies == null)
			{
				goodrow[nrow] = true;
				//this id is new
	 ...
	 *If btakelog is true then this the log base 2 of a value over the
	 *time point 0 value. If it is false then this is the difference
	 *with the time point 0 value.
	 */
	public DataSetCore logratio2()
	{
		...
		for (int nrow = 0; nrow < numrows; nrow++)
		{
			...
			else
			{
				for (int ncol = 1; ncol < numcols; ncol++)
				{
					if (btakelog)
					{
						data[nrow][ncol] = Math.log(data[nrow][ncol]/dnormval)/DLOG2;
					}
					else
					{
		...
		return new DataSetCore(szInputFile,genespottimedata,generepeatspottimedata,
				genespottimepma, generepeatspottimepma,
				data, pmavalues, probenames,genenames,
				dthresholdvalue, btakelog,bspotincluded,badd0,
				dsamplemins, sortedcorrvals,nmaxmissing,
				dmincorrelation, htFiltered,	
				szProbeHeader, szGeneHeader,
	...
	/////////////////////////////////////////////////////////////////////////////////
	/**
	 *Given an array of otherDataSets, merges it with the current data set
	 *by storing in data the median of the values. If bfullrepeat is
	 *true then stores the repeat and missing data into generepeatspottimedata and
	...
	public DataSetCore mergeDataSets(DataSetCore[] otherDataSets)
	{
		...
		{
			//the repeats are of the individual time points for which we will take the mean? median
			...
			for (int ncol = 0; ncol < numcols; ncol++)
			{
				...
				for (int ndataset = 0; ndataset < otherDataSets.length;ndataset++)
				{
					...
					if (otherDataSets[ndataset].pmavalues[nrow][ncol]!=0)
					{
						...
						npma = Math.max(npma,otherDataSets[ndataset].pmavalues[nrow][ncol]);
					}
				}
		...
		DataSetCore mergedSet = new DataSetCore(szInputFile,
				genespottimedata,generepeatspottimedata,
				genespottimepma, generepeatspottimepma,
				mergedata,mergepmavalues,
				probenames,genenames,dthresholdvalue, btakelog,
				bspotincluded,badd0, dsamplemins,
				sortedcorrvals,nmaxmissing,
				dmincorrelation, htFiltered,
				szProbeHeader, szGeneHeader,	bmaxminval,
				otherInputFiles,bfullrepeat);
	...
	////////////////////////////////////////////////////////////////////////////////
	/**
	 *Computes the average pairwise correlation between gene repeats
	 *stores it in sortedcorrvals. Filters those genes which do not have
	 *a sortedcorrvals exceeding dmincorrelation
	 */
	...
	{		        
		...
		for (int nrow = 0; nrow < sortedcorrvals.length; nrow++)
		{
			...
			for (int nrepeatset = 0; nrepeatset < RepeatSet.length; nrepeatset++)
			{
				...
				Util.correlation(theDataSet1.data[nrow],RepeatSet[nrepeatset].data[nrow],
						theDataSet1.pmavalues[nrow],RepeatSet[nrepeatset].pmavalues[nrow]);
				for (int nrepeatset2 = nrepeatset+1; nrepeatset2 < RepeatSet.length; nrepeatset2++)
				{
					sortedcorrvals[nrow] += dweight*
					Util.correlation(RepeatSet[nrepeatset].data[nrow],RepeatSet[nrepeatset2].data[nrow],
							RepeatSet[nrepeatset].pmavalues[nrow],RepeatSet[nrepeatset2].pmavalues[nrow]);                         
				}
			}
	...
	public DataSetCore filtergenesgeneral(boolean[] keepgene, int nkeep, boolean bstore) 
	{
		...
		boolean bfiltergenespottime = ((genespottimedata != null)&&
				(genespottimedata.length == numrows));
		boolean bfiltergenerepeatspottime = ((generepeatspottimedata!= null)&&
				(generepeatspottimedata.length == numrows));
		...
		return new DataSetCore(szInputFile,filtergenespottimedata,filtergenerepeatspottimedata,
				filtergenespottimepma, filtergenerepeatspottimepma, 
				filterdata, filterpmavalues,  filterprobenames,
				filtergenenames,dthresholdvalue, btakelog,
				bspotincluded,badd0, dsamplemins,
				sortedcorrvals,nmaxmissing,
				dmincorrelation, htFiltered,
				szProbeHeader, szGeneHeader,
	...
	private DataSetCore filtergenesthreshold2change()
	{
		...
		for (int nrow = 0; nrow < numrows; nrow++)
		{
			...
			for (int ncol = 1; ncol < numcols; ncol++)
			{ 
				if ((pmavalues[nrow][ncol] > 0)&& (Math.abs(data[nrow][ncol]) > dmaxval))
				{
