...
/**
 * Class for a table summarizing the TF-gene enrichments for a selected set of gene
 * This is the table that comes up when pressing the TF summary button
 */
public class DREMGui_StaticSummaryTable extends JPanel implements ActionListener 
{
	 ...
	 */
	 public DREMGui_StaticSummaryTable(JFrame theFrame,DREM_DataSet theDataSet,DREM_Timeiohmm theTimeiohmm,
			 int[][] bindingpvalTF,int[][] bindingpvalTFindex,
			 String[] tfNames, boolean[] include, int numgenes)
	 {
		 ...
		 for (int nrow = 0; nrow < numtf; nrow++)
		 {
			 for (int nel = 0; nel < theTimeiohmm.dBindingSigns.length; nel++)
			 {
				 if (theTimeiohmm.dBindingSigns[nel] != 0)
				 {
					 ...
					 int ntotal = theTimeiohmm.filteredClassifier.nBaseCount[nrow][theTimeiohmm.filteredClassifier.getFeatureIndex(theTimeiohmm.dBindingSigns[nel])];
					 ...
					 for (int ngeneindex = 0; ngeneindex < bindingpvalTFindex[nrow].length; ngeneindex++)
					 {
						 ...
						 if ((include[ngene])&&(bindingpvalTF[nrow][ngeneindex]==theTimeiohmm.dBindingSigns[nel]))
						 {
							 ncount++;
						 }
					 }
					 ...
					 dexpect = ntotal*numgenes/(double)theTimeiohmm.ntotalcombined;

					 tabledata[nrowindex][3] = nf2.format(dexpect);
					 double ddiff =   ncount- dexpect;
					 tabledata[nrowindex][4] = nf2.format(ddiff);
					 tabledata[nrowindex][5] = ""+ DREMGui_EdgeTable.doubleToSz(StatUtil.hypergeometrictail(ncount-1, 
							 ntotal, theTimeiohmm.ntotalcombined-ntotal,numgenes));
	 ...
	 public void actionPerformed(ActionEvent e)
	 {
		 ...
		 else if (szCommand.equals("help"))
		 {
			 ...
			 String szMessage = 
				 "A TF-summary table provides aggregate TF-gene interaction information "+
