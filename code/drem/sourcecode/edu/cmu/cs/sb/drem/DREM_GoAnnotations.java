...
public class DREM_GoAnnotations extends GoAnnotations
{
	...
	DREM_GoAnnotations(String szorganismsourceval,String szxrefsourceval,
		...
		super(szorganismsourceval,szxrefsourceval,szxrefval, szGoFile, szGoCategoryFile, 
	...
	public void buildRecDREM(DREM_Timeiohmm.Treenode treeptr,String[] genenames)
	{
		...
		while(itrSet.hasNext())
		{
			...
			theRecIDdrem[nindex] = new RecIDdrem(szID,szName,
					dminoverallpval,dminsplitpval);
			nindex++;
	...
	public void findmins(HashMap htGOtoMinSplit, HashMap htGOtoMinOverall,
			DREM_Timeiohmm.Treenode treeptr,String[] genenames,HashMap htParentGO)
	{
		if (treeptr != null)
		{
			...
			for (int ngene = 0; ngene < treeptr.bInNode.length; ngene++)
			{
				if (treeptr.bInNode[ngene])
				{
					...
					while (theiterator.hasNext())
					{
						...
						{
							htGOtoCount.put(szid, new Integer(nobjcount.intValue()+1));
						}
					}
				}
			}
			...
			while (entryIterator.hasNext())
			{
				...
				if ((dminpvalobj==null)||(dpval < dminpvalobj.doubleValue()))
				{
					htGOtoMinOverall.put(szGO, new Double(dpval));
				}

				if ((treeptr.parent != null) && (treeptr.parent.numchildren >=2))
				{
					int nparentcategory = ((Integer) htParentGO.get(szGO)).intValue();

					double dpvalsplit = StatUtil.hypergeometrictail(nval,nparentcategory,  
							treeptr.parent.numPath-nparentcategory, treeptr.numPath);

					Double dminpvalsplitobj = (Double) htGOtoMinSplit.get(szGO);
					if ((dminpvalsplitobj==null)||(dpvalsplit < dminpvalsplitobj.doubleValue()))
					{
			...
			{
				findmins(htGOtoMinSplit, htGOtoMinOverall,treeptr.nextptr[nchild],genenames,htGOtoCount);
			}
		}
	}
	...
	public static class RecIDdrem
	{
		...
		public RecIDdrem(String szID, String szName, double dminoverallpval, double dminsplitpval)
		{
			this.szID = szID;
			this.szName = szName;
