...
/**
 * Class encapsulates a gene table which is used to 
 * display the selected genes in DREM
 */
public class DREMGui_GeneTable extends JPanel implements ActionListener 
{
	 ...
	 */
	public DREMGui_GeneTable(JFrame theFrame,DREM_DataSet theDataSet,DREM_Timeiohmm theTimeiohmm,
			int[][] bindingpval,int[][] bindingpvalIndex,
			int[][] bindingpvalTF,int[][] bindingpvalTFIndex,
			String[] tfNames, boolean[] include)
	{
	...
	private void geneTableHelper() 
	{
			...
			if (include[ngene])
			{
				...
				for (int ncol = 0; ncol < theDataSet.numcols; ncol++)
				{
					...
					else
					{
						...
						ncount[ncol]++;
						tabledata[ntableindex][ncol+2]= nf2.format(theDataSet.data[ngene][ncol]);
					}
				}
				...
				for (int ncol = 0; ncol < bindingpvalIndex[ngene].length; ncol++)
				{
					tabledata[ntableindex][bindingpvalIndex[ngene][ncol]
					                                               +nstart]= ""+(int) bindingpval[ngene][ncol];
				}
				ntableindex++;
			}
		}
		...
		for (int ni = 0; ni < davg.length; ni++)
		{
			...
			dstd[ni] = Math.sqrt((dsumsq[ni]-ncount[ni]*davg[ni]*davg[ni])/(ncount[ni]-1));
		}
		...
		if (szCommand.equals("staticsummary"))
		{
			javax.swing.SwingUtilities.invokeLater(new Runnable() 
			{
				public void run() 
				{
					...
					DREMGui_StaticSummaryTable newContentPane = 
						new DREMGui_StaticSummaryTable(theFrame,theDataSet,
								theTimeiohmm,bindingpvalTF,bindingpvalTFIndex,
								tfNames,include,ntabledatasize); 
