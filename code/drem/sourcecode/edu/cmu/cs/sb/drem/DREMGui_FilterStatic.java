...
/**
 * Class encapsulates window used to select a subset of genes based on the static transcription factors
 * in the input to regulate the gene. 
 */
public class DREMGui_FilterStatic extends JPanel implements ActionListener, ItemListener, ChangeListener
{
			...
			GoAnnotations tga,DREM_Timeiohmm.Treenode rootptr)
	{
		...
		numcols =theTimeiohmm.hsUniqueInput.size()+1;

		tfVals = theTimeiohmm.dBindingSigns;
		...
		table.setPreferredScrollableViewportSize(new Dimension(425, 
		...
		buttonPanel0.add(new JLabel("Genes selected must meet constraints for"));
		...
		JLabel theTopLabel = new JLabel("  Only display enrichments with a score less than 10^-X where X is:");
		...
		pvalLabel = new JLabel("X = "+ninitval/10.0+
				"; score threshold is "+ DREMGui_KeyInputs.doubleToSz(Math.pow(10,-ninitval/10.0)));
	 ...
	 */
	void setTFSetLabelTextVisible(DREM_Timeiohmm.Treenode treeptr, boolean bvisible)
	{
		if (treeptr != null)
		{
	...
	public void actionPerformed(ActionEvent e) 
	{
		...
		{
			unapplyButton.setEnabled(false);
			for (int ngene = 0; ngene < theTimeiohmm.bindingSignGene.length; ngene++)
			{
				theDREMGui.bTFVisible[ngene] = true;
				boolean bvisible = theDREMGui.bglobalVisible&& theDREMGui.bPathVisible[ngene]&&
				theDREMGui.bGOVisible[ngene]&&theDREMGui.bSetVisible[ngene];
		...
		{
			String szMessage = 
				"This dialog box allows the selection of a subset of genes based on being regulated "+
			...
			if (bintersect)
			{
				...
				for (int ngene = 0; ngene < theTimeiohmm.bindingSignGene.length; ngene++)
				{
					...
					while ((nrow < numrows) && (bsatisfied))
					{
						...
						int nhit = Arrays.binarySearch(theTimeiohmm.bindingGeneindex[ngene],nrow);
						double dval;
						if (nhit < 0)
						{
							dval = 0;
						...
						{
							dval = theTimeiohmm.bindingSignGene[ngene][nhit];
						}
					...
					else
					{
						theDREMGui.bTFVisible[ngene] = true;
						boolean bvisible = theDREMGui.bglobalVisible && theDREMGui.bPathVisible[ngene]
						                                                                        &&  theDREMGui.bSetVisible[ngene] && theDREMGui.bGOVisible[ngene];
						theDREMGui.plArray[ngene].setVisible(bvisible); 
						theDREMGui.plArray[ngene].setPickable(bvisible);
					}
				}
		...
		if ((treeptr != null)&&(bapplyset))
		{
			...
			if (!bsplitgolabels)
			{
				...
				if (boveralltfeligible)
				{
					...
					dpval = StatUtil.hypergeometrictail(nmatch-1, 
							nsumfullel, theTimeiohmm.ntotalcombined-nsumfullel,nsumthispath);


					if (dpval <= dsetpval)
					{
						treeptr.sztfsetlabel = nmatch+";"+Util.doubleToSz(dpval);
			...
			else if ((treeptr.parent != null) && (treeptr.parent.numchildren >= 2))
			{
				int nparentbase =0;
	...
	public void stateChanged(ChangeEvent e) 
	{
		...
		if (!source.getValueIsAdjusting()) 
		{
			...
			pvalLabel.setText("X = "+source.getValue()/10.0+
					"; score threshold is "+DREMGui_KeyInputs.doubleToSz(dsetpval));
			addSetLabels(rootptr);
		}
	}
}