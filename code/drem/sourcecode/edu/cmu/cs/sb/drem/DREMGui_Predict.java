...
/**
 * Class encapsulates a window to specify an input for each transcription factor,
 * then displays a probability of transitioning to each hidden state
 */
public class DREMGui_Predict extends JPanel implements ActionListener
{
	 ...
	 */
	public DREMGui_Predict(JFrame theFrame,DREMGui theDREMGui,final DREM_Timeiohmm.Treenode rootptr)
	{
		...
		conditionalBox = 
			new JCheckBox("Probabilities should be conditional on gene not being filtered");
	...
	/**
	 * Sets whether the probability on each hidden state is visible or not based on bvisible
	 */
	...
	public void actionPerformed(ActionEvent e) 
	{
		...
		if (szcommand.equals("show"))
		{
			...
			for (int nrow = 0; nrow < numrows; nrow++)
			{
				for (int ncol = 1; ncol< numcols; ncol++)
				{
					if (((JRadioButton)hmst.getValueAt(nrow,ncol)).isSelected())
					{
			...
			try
			{
				...
				{
					double[] dx;
					dx = theTimeiohmm.filteredClassifier.distributionForInstance(predictinputs);
					dprob = dx[1];
				} 
		...
		else if (szcommand.equals("default"))
		{
			...
			for (int nrow = 0; nrow < numrows; nrow++)
			{
				((JRadioButton)hmst.getValueAt(nrow,nmidcol)).setSelected(true);
			}
			repaint();
		}
		...
		{
			String szMessage = 
				"DREM allows one to view for any set of transcription factor-gene regulation interaction inputs, "+
