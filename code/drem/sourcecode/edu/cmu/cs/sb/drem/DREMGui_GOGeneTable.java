...
/**
 * Class encapsulates a table of genes all assigned to the same GO category
 * when clicking on a GO category in a GOTable
 */
	 ...
	 */
	 ...
	 private void loadTable()
	 {
		 ...
		 for (int ngeneindex = 0; ngeneindex < include.length; ngeneindex++)
		 {
			 ...
			 {
				 Vector rec = new Vector(2); 

				 HashSet goList;
				 String szid;

				 goList = theDataSet.tga.labelsForID(theDataSet.genenames[ngeneindex]);
				 ...
				 {       
					 ...
					 for (int ncol = 0; ncol < tfNames.length; ncol++)
					 {
						 ...
						 {
							 rec.add(""+(int) bindingpval[ngeneindex][nhitindex]);
						 }
						 else
						 {
							 rec.add("0");
	 ...
	 public void actionPerformed(ActionEvent e)
	 {
				 ...
				 public void run() 
				 {
					 ...
					 DREMGui_StaticSummaryTable newContentPane = 
						 new DREMGui_StaticSummaryTable(theFrame,theDataSet,
								 theTimeiohmm,bindingpvalTF,
								 bindingpvalTFIndex,tfNames,
								 goinclude,numgoinclude); 
		 ...
		 else if (szCommand.equals("select"))
		 {
			 ...
			 {
				 ((DREMGui_GOFilter) theDREMGui.filterGOFrame.getContentPane()).rowSM.clearSelection();
			 }
		 }
